...
>
> **特别注意**：当你发现 Step 方案有误、不完整或难以执行时，**不要在对话中说「我建议」「我认为应该」「让我设计」这类话**。你的任务是在回执中如实记录问题，然后停止。规划层会修正方案后重新下发。你不是「合作者」——你是「执行者」。
...
### 11.1 沟通优先原则（硬约束 · Superpowers 对齐）

- **默认沟通模式**：用户输入若无明确「执行/实现/落地/写代码/出方案」等执行指令，
  一律视为**需求沟通**。AI 的职责是反复确认、追问、细化需求，直到信息完备度 ≥ 90%，
  才可提出执行方案供用户确认。
- **执行触发条件（满足任一即可进入执行模式）**：
  - 用户明确说「执行」「实现」「开始写」「落地」「出代码」「按这个方案做」
  - 用户在沟通多轮后说「可以了」「没问题」「就按这个来」
  - 用户主动给出了足够详细的执行指令（字段、类型、边界条件、错误码等均已明确）
- **沟通模式的输出要求**：
  - 以提问、确认、选项对比为主；**不出完整代码**（可出 ≤ 5 行的关键签名/接口草稿）
  - 每轮沟通必须**显式标注当前信息完备度估算**（如「当前信息量约 60%」），
    并列出**已明确**与**待确认**清单
  - 信息完备度 ≥ 90% 时，主动提示：「需求已足够清晰，是否进入执行方案？」
- **例外**：明确的 bug 修复（有复现步骤 + 错误日志）、纯代码审查、纯答疑可不走沟通模式
