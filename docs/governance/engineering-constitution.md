...
你的职责边界：
- ✅ 接收规划层下发的**需求方向文档**（目标/非目标/影响范围/风险），**自行拆分 Step、设计每个 Step 的执行/测试方案**并自主执行（根目录 `system.md` §0.6 自主闭环）
- ✅ 编译、运行测试，产出结构化执行回执和测试回执（按根目录 `system.md` §7 格式）
...
执行角色在自主闭环中按任务复杂度决定处理强度（根目录 `system.md` §2），不涉及任何具体模型：
