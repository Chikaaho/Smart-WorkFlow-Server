...
@Profile("dev")
@Component
public class WorkflowVerificationRunner implements CommandLineRunner {

    private static final Logger log = LoggerFactory.getLogger(WorkflowVerificationRunner.class);

    private final ExternalDatasourceService dsService;
    private final SqlExecutor sqlExecutor;
    private final SqlExecutionAuditService auditService;

    public WorkflowVerificationRunner(ExternalDatasourceService dsService,
    ...
    public void run(String... args) {
        ...
        // ---- 测试 2: DROP TABLE 被拒 ----
        try {
            sqlExecutor.execute(dsId, "DROP TABLE wf_external_datasource", operatorId, operatorName);
        ...
        // ---- 测试 3: UPDATE 被拒 ----
        try {
            sqlExecutor.execute(dsId, "UPDATE wf_external_datasource SET name='hacked'", operatorId, operatorName);
        ...
        // ---- 测试 4: 堆叠语句 a;b 被拒 ----
        try {
            sqlExecutor.execute(dsId, "SELECT 1; DROP TABLE wf_external_datasource", operatorId, operatorName);
