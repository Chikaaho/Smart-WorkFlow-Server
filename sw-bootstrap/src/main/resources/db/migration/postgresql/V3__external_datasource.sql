...
comment on table wf_external_datasource is '外部数据源连接信息';
comment on column wf_external_datasource.password_cipher is 'AES-256-GCM 加密密码密文';
comment on column wf_external_datasource.read_only is '是否只读：1=是，0=否（建议配合只读账号）';
comment on column wf_external_datasource.enabled is '是否启用：1=启用，0=禁用';
...
comment on table wf_sql_execution_audit is 'SQL 执行审计日志';
comment on column wf_sql_execution_audit.sql_text is '执行的 SQL 原文';
comment on column wf_sql_execution_audit.success is '执行结果：0=失败，1=成功';
