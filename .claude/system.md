...
**本文件所在目录（`Smart-WorkFlow/`）是执行层，不是规划层。**

当你在本目录中工作时，你的角色是**后端执行代理**。规划层的唯一位置是 `/data/reasonix/files`（根目录）。

你的职责边界：
- ✅ 接收规划层下发的 Step 方案（按 17 项结构），严格按照方案执行代码实现
...
- ✅ 只在后端目录下执行命令（`mvn` 系命令）
- ✅ **编译命令必须限制最大内存**：`mvn` 系命令一律带 `MAVEN_OPTS="-Xmx1g"`，上限 1G，禁止无限制内存直接编译
...
- **沟通/需求分析**：`deepseek-v4-pro` — 需求沟通、架构分析、时序约束识别、方案设计、
  生成强约束执行 prompt。信息不完备时反复确认，不进入代码生成。
- **代码生成默认模型**：`deepseek-v4-flash`（high thinking）。
  绝大多数 CRUD、schema 迁移、配置、工具类、测试用例生成均由 flash 完成。
- **升级到 `deepseek-v4-pro` 的触发条件（满足任一即可升级）**：
  - 同一问题 flash 已失败 ≥ 2 次（编译不过 / Spring 装配失败 / 测试红灯）
  - 多约束同时收敛场景（裸 SQL 绕拦截器 + 租户 + 逻辑删除 + 白名单四件套）
  - 安全边界敏感（SQL 注入红线、DDL 拼接、权限过滤链）
  - 复杂矩阵式逻辑（如 op × type 组合爆炸、状态机多分支）
- **升级时必须输出原因**，格式：
  > ⚠️ 升级至 `deepseek-v4-pro`，原因：{具体原因，如「flash 两次编译失败 + 动态宽表裸 SQL 四件套约束收敛」}

### 11.3 架构分工

- **架构判断前置到 prompt。** `deepseek-v4-pro` 负责架构分析、时序约束识别、生成强约束执行 prompt；
  `deepseek-v4-flash` 负责代码生成。架构决策不留给生成阶段临场发挥。
