# system.md — Smart-WorkFlow 项目约束（后端）
...
本目录会话角色为**执行（后端）**，由用户在会话开始时声明（根目录 `system.md` §0.2）；规划角色的唯一位置是 `/data/reasonix/files`（根目录）。
...
- ✅ 只在后端目录下执行命令（`mvn` 系命令）
- ✅ **编译命令必须限制最大内存**：`mvn` 系命令一律带 `MAVEN_OPTS="-Xmx512m"`，上限 512M，禁止无限制内存直接编译
